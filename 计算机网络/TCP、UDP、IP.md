...
UDP (UserDatagramProtocol)是一种无连接的、轻量级的传输层协议，尽管UDP提供标头和有效负载的完整性验证（通过校验和），但它不保证数据传输的可靠性，并且UDP层在发送后不会保留UDP消息的状态。

UDP优点：传输速度快、实时性高、开销小
UDP缺点：没有流控制，没有应答确认机制，不能解决丢包、重发、错序问题，可靠性差。

> 并不是所有使用UDP协议的应用层都是不可靠的，应用程序可以自己实现可靠的数据传输，通过增加确认和重传机制；UDP协议最大的特点就是速度快。

### UDP数据报

![20230407113122-2023-04-07](https://blog-1256965811.cos.ap-guangzhou.myqcloud.com/img/20230407113122-2023-04-07.png)

#### source port、destination port

分别占2字节。
源端口和目的端口，代表数据报的是哪个端口发出的，以及要发送的目的端口。与TCP报文段相同的是，UDP数据报中不含IP地址信息，因为那是网络层需要考虑的事情。

源端口是可选字段，因为UDP不需要应答。

**端口**

端口号占2个字节，一个字节为8位，所以是16位的非负整数，范围是0-65535（2^16）之间，这个范围会分为三种不同的端口号段，由互联网分配号码管理局（IANA）进行分配

- 周知/标准端口号，范围是0-1023。在Unix操作系统上，使用这些端口之一需要超级用户（root）操作权限
- 注册端口号，范围是1024-49151.用于注册IANA注册服务的注册端口
- 私有端口号，范围是49152-65535.未正式指定用于任何特定服务，可用于任何目的。这些端口也可以用作临时端口，在主机上运行的软件可以使用这些端口，根据需要动态创建通信终结点。

端口的作用，简单说就是为了区分不同应用程序，当电脑接收到一个数据报，将根据不同的端口将数据送给不同的应用程序。

#### Length

占用两个字节，标识UDP头的长度，包括首部长度和数据长度。可以有65535字节那么长。但是一般网络在传送的时候，一次一般传送不了那么长的协议（涉及MTU问题）

#### Checksum

校验和字段用于发现头部信息和数据中的传输错误。这是一个可选项，并不是所有的系统都对UDP数据包加以校验和数据（相对TCP协议的必须来说），但是RFC中标准要求，发送端应该计算校验和。

![20230407162615-2023-04-07](https://blog-1256965811.cos.ap-guangzhou.myqcloud.com/img/20230407162615-2023-04-07.png)

UDP和TCP都包含一个伪首部，这是为了计算校验和而设置的。伪首部设置还包含IP地址这样的IP协议里面都有的信息，目的是让UDP两次检查数据是否已经正确到达目的地。如果发送端没有打开校验和选项，而接受端计算校验和有差错，那么UDP数据将会被悄悄的丢掉（不保证送达），而不产生任何差错报文。

### UDP适用场景

UDP应用一般作为流媒体应用、语音交流、视频会议所使用的传输层协议，还有许多基于互联网电话服务使用的VOIP（基于IP的语音）也是基于UDP运行的，实时视频流和音频流应用程序旨在处理偶尔丢失、错误的数据包，因此只会发生质量轻微下降，而避免了重传数据包带来的高延迟。

DNS协议底层也使用了UDP协议，主要是因为查询阶段必须快速。采用UDP协议时，只要应用进程将数据传给UDP，UDP就会将此数据打包进UDP报文段并立即传递给网络层，然后TCP用拥塞控制的功能，它会在发送前判断互联网的拥堵情况，如果互联网极度阻塞，那么就会抑制TCP的发送方。

额外的说，每个TCP报文段都有20字节的首部开销，而UDP仅仅只有8字节的开销。

### UDP洪水攻击

UDP洪水攻击（UDP flood attack）是一种拒绝服务攻击，攻击者将大量用户数据报协议（UDP）数据包发送到目标服务器，旨在让该设备的处理和响应能力无力承担。由于UDP洪水攻击，保护目标服务器的防火墙也可能不堪重负，导致对正常的流量拒绝服务。

在正常情况下，服务器在特定端口上收到UDP数据包时，将通过以下两个步骤进行响应：

- 服务器首先检查是否有任何当前监听指定端口请求的程序正在运行
- 如果该端口上没有程序正在接收数据包，则服务器以ICMP（ping）数据包作为响应，以告知发送方目标不可达。

由于目标服务器利用资源来检查并响应每个接收到的UDP数据包，当收到大量UDP数据包时，目标资源会很快耗尽，从而对正常流量拒绝服务。其主要特点是攻击者可以伪造源IP地址，使得攻击流量难以被追踪。

如何防护UDP洪水攻击？常见防御措施包括限制UDP数据包的发送速率、使用网络设备进行流量过滤、使用防火墙和入侵检测系统等安全设备进行防御。
...
|服务对象|TCP一对一；UDP多对多|
|可靠性|TCP可靠；UDP不可靠|
|流量/拥塞控制|TCP有流量控制和拥塞控；UDP无控制|
|首部开销|TCP20字节（以上）；UDP只有固定8个字节|
|传输方式|TCP流式传输、没有边界；UDP数据包的形式|
|包长度字段 Length|TCP没有该字段、可利用IP总长度-IP首部-TCP首部长度求得；UDP2字节的包长度字段|
|分片不同|TCP大于MSS时，在传输层进行分片，丢失时重传；UDP大于MTU时，在IP层进行分片，丢失时不重传|
|应用场景|TCP适用于FTP文件传输等对可靠性要求高的场景；UDP适用于流媒体、即时通信等对延迟性要求高的场景|
...
- Linux 高性能服务器编程
- [UDP协议详解](https://mp.weixin.qq.com/s?__biz=MzIxNTg1NzQwMQ==&mid=2247485337&idx=1&sn=e1604749697a70b8dd9276b9327113d8&chksm=9790a60fa0e72f19a9eab3a19685f455e2fa4ce01910a7806ffabcbe1a7d6513b46fe1edbbf2&scene=178&cur_album_id=1611732790255927298#rd)
