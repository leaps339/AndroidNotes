...
标志位，占用8个比特。每一位的值只有0和1，分别表达不同的意思，代表包的类型，主要用于操纵TCP的状态机。

- CWR（Congestion Window Reduced）：表示收到了一个拥塞通知（ECN）的 ECE 标志，并已经减少了发送窗口的大小。
- ECE（ECN Echo）：占用 1 个比特，表示接收方实现了 ECN（Explicit Congestion Notification）。CWR和ECE标志位可以用于网络拥塞的控制，可以更加准确地检测网络中的拥塞，并采取相应的措施以减少拥塞。
...
是可变长的可选信息。这部分最多包含40字节，因为TCP头部最长是60字节，前面是固定的20字节。

一般用于提供额外的信息或控制。例如最大段大小（MSS，Maximum Segment Size），窗口缩放因子等。

**填充 Padding**

用于填充TCP数据包，使其长度达到4字节的倍数。
